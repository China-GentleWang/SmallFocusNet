...
def calculate_metrics(cm):
    ...
    return np.mean(sensitivity), np.mean(specificity), np.mean(f1_scores)
...
def bootstrap_confidence_interval(y_true, y_pred, num_bootstraps=1000, alpha=0.05):
    ...
    for _ in range(num_bootstraps):
        ...
        sensitivity, specificity, f1_score_avg = calculate_metrics(cm)

        bootstrapped_sensitivities.append(sensitivity)
        bootstrapped_specificities.append(specificity)
        bootstrapped_f1_scores.append(f1_score_avg)

    sensitivity_ci = np.percentile(bootstrapped_sensitivities, [100 * alpha / 2, 100 * (1 - alpha / 2)])
    specificity_ci = np.percentile(bootstrapped_specificities, [100 * alpha / 2, 100 * (1 - alpha / 2)])
    f1_score_ci = np.percentile(bootstrapped_f1_scores, [100 * alpha / 2, 100 * (1 - alpha / 2)])

    return sensitivity_ci, specificity_ci, f1_score_ci
...
def _test_model(model_path, train_file, classes=3, batch_size=1):
    ...
    overall_sensitivity, overall_specificity, overall_f1_score = calculate_metrics(cm)
    sensitivity_ci, specificity_ci, f1_score_ci = bootstrap_confidence_interval(np.array(total_targets), np.array(total_preds))

    print(f'Overall Sensitivity: {overall_sensitivity:.4f}, 95% CI: [{sensitivity_ci[0]:.4f}, {sensitivity_ci[1]:.4f}]')
    print(f'Overall Specificity: {overall_specificity:.4f}, 95% CI: [{specificity_ci[0]:.4f}, {specificity_ci[1]:.4f}]')
    print(f'Overall F1 Score: {overall_f1_score:.4f}, 95% CI: [{f1_score_ci[0]:.4f}, {f1_score_ci[1]:.4f}]')
